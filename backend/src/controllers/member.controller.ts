...
export const updateMemberController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Add text fields if they were sent
...
export const deleteMemberController = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (error.code === 'P2003') { // Prisma foreign key constraint fail (onDelete: Restrict)
            res.status(400).json({ message: "Cannot delete member.", details: "This person is still assigned to one or more projects." });
        } else {
